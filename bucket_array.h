...
struct Bucket
{
    SArr<bool, items_max> occupied; // @Important: Remember to clear this (contains garbage value)
    SArr<Type, items_max> data;
...
Bucket<T, items_max> *add_bucket(Bucket_Array<T, items_max> *array)
{
    ...
    Bucket<T, items_max> *new_bucket = New<Bucket<T, items_max>>();
    ...
    new_bucket->occupied.count = items_max;
    new_bucket->data.count     = items_max;
...
void bucket_array_reset(Bucket_Array<T, items_max> *array)
{
    for (auto it : array->all_buckets) my_free(it, array->allocator);
