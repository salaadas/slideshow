...
inline
void array_add_if_unique(RArr<String> *array, String to_add)
{
    auto found = array_find(array, to_add);

    if (found != NULL) return;

    array_add(array, to_add);
}
    ...
    for (auto &info : directories)
    {
        if (event->wd == info.watch_descriptor)
        {
            ...
            if (ext.count)
            {
                ...
                {
                    if ((change.short_name == short_name) && (change.full_name == full_name) &&
...
bool hotloader_process_change()
{
    ...
    for (auto &change : asset_changes)
    {
        ...
        // @Incomplete: not handling catalogs reload for now...

        auto handled = false;

        // Do stuff with catalogs and set handled=true if handled there
        ...
        my_free(change.short_name.data);
        my_free(change.full_name.data);
